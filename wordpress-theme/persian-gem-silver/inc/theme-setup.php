...
function pgs_seed_sample_products()
{
    ...
    $samples = [
        ['انگشتر نقره مردانه', 'ترکیب نقره ۹۲۵ با نگین مشکی.', '3200000'],
        ['انگشتر نقره زنانه', 'طراحی ظریف مناسب استایل روزانه.', '2750000'],
        ['دستبند نقره کلاسیک', 'دست‌ساز با قفل مقاوم.', '1980000'],
        ['گردنبند سنگ طبیعی', 'ترکیب زنجیر نقره و سنگ فیروزه.', '2490000'],
    ];

    foreach ($samples as $sample) {
        $product = new WC_Product_Simple();
        $product->set_name($sample[0]);
        $product->set_status('publish');
        $product->set_catalog_visibility('visible');
        $product->set_description($sample[1]);
        $product->set_short_description($sample[1]);
        $product->set_regular_price($sample[2]);
        $product->save();
    }
}
